...
class IssuedReportRepository @Inject constructor(
    ...
    suspend fun create(issuedReport: IssuedReport): Response<Response.Action> {
        return try {
            ...
            val data = IssuedReportWorker.convert(issuedReport)
            val workRequest = OneTimeWorkRequestBuilder<IssuedReportWorker>()
                .addTag(issuedReport.issuedReportId)
                .setInputData(data)
                .build()
            workManager.enqueueUniqueWork(issuedReport.issuedReportId,
